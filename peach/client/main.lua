...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.JuiceDealer.coords, true) < 1.50 then
...
function OpenJuiceShop()
	...
	for k, v in pairs(ESX.GetPlayerData().inventory) do
		local price = Config.JuiceDealerItems[v.name]
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'juice_shop', {
		title    = _U('dealer_title'),
		align    = 'top-left',
		elements = elements
	}, function(data, menu)
		TriggerServerEvent('esx_farm:sellJuice', data.current.name, data.current.value)
